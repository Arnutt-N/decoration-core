<template>
  <div class="space-y-6 animate-fade-in">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">รายการคำขอเครื่องราชฯ ช้างเผือก-มงกุฎไทย</h1>
        <p class="text-gray-500 mt-1">รายการคำขอพระราชทานเครื่องราชอิสริยาภรณ์ทั้งหมด</p>
      </div>
    ...
    <!-- Empty -->
    <EmptyState v-else title="ไม่พบรายการคำขอ" description="ยังไม่มีคำขอเครื่องราชอิสริยาภรณ์ในระบบ" />
  </div>
</template>
...
async function fetchData() {
  ...
  try {
    ...
    const data = await api.get(`/decorations?${params}`)
