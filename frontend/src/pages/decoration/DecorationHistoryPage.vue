<template>
  <div class="space-y-6 animate-fade-in">
    <div>
      <h1 class="text-2xl font-bold text-gray-900">ประวัติการได้รับเครื่องราชอิสริยาภรณ์</h1>
      <p class="text-gray-500 mt-1">ค้นหาและดูประวัติการได้รับพระราชทานเครื่องราชอิสริยาภรณ์</p>
    </div>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="(item, idx) in history" :key="item.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-sm text-gray-700">{{ item.decoration_type }}</td>
    ...
    <EmptyState v-else-if="!loading && searched && !history.length" title="ไม่พบประวัติ" description="ไม่พบข้อมูลประวัติเครื่องราชอิสริยาภรณ์ของบุคลากรท่านนี้" />
  </div>
</template>
...
async function fetchHistory(person) {
  ...
  searched.value = true
  try {
    const data = await api.get(`/decorations/history/${person.id}`)
