<template>
  <div class="space-y-6 animate-fade-in">
    <div>
      <h1 class="text-2xl font-bold text-gray-900">ประวัติได้รับเหรียญจักรพรรดิมาลา</h1>
      <p class="text-gray-500 mt-1">ค้นหาและดูประวัติการได้รับพระราชทานเหรียญจักรพรรดิมาลา</p>
    </div>
    ...
    <div v-if="selectedPerson && !loading" class="card max-w-xl">
      ...
      <p class="text-sm text-gray-500">{{ selectedPerson.position }} | {{ selectedPerson.department }}</p>
    </div>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">ประเภทเหรียญ</th>
            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">ปีที่ได้รับ</th>
            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">ราชกิจจาฯ</th>
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="(item, idx) in history" :key="item.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-sm text-gray-900 font-medium">{{ item.medal_type || 'เหรียญจักรพรรดิมาลา' }}</td>
            <td class="px-4 py-3 text-sm text-gray-700">{{ item.award_year }}</td>
            <td class="px-4 py-3 text-sm text-gray-500">{{ item.gazette_date || '-' }}</td>
            <td class="px-4 py-3 text-sm text-gray-500">{{ item.remarks || '-' }}</td>
          </tr>
        </tbody>
      </table>
    </div>

    <EmptyState v-else-if="!loading && searched && !history.length" title="ไม่พบประวัติ" description="ไม่พบข้อมูลประวัติเหรียญจักรพรรดิมาลาของบุคลากรท่านนี้" />
  </div>
</template>
...
async function fetchHistory(person) {
  ...
  searched.value = true
  try {
    const data = await api.get(`/chakrabardi/history/${person.id}`)
